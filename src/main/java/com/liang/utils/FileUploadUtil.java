...
public class FileUploadUtil {
    ...
    public String fileUpload(MultipartFile file, String type) throws Exception {
        ...
        String fileDir = rootPath + File.separatorChar + type;
        File dir = new File(fileDir);
        if (!dir.exists()) {
            dir.mkdirs();
        }

        String storePath = generateStorePath(type, fileName);
        String result = "";
        result = storePath.substring(storePath.indexOf(":") + 1, storePath.length());
    ...
    public String generateStorePath(String folder, String fileName){
        return rootPath + File.separatorChar + folder + File.separatorChar + fileName;
    }
